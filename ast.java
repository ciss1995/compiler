...
class DeclListNode extends ASTnode {
    ...
    public void nameAnalysis(SymTable symTab, int offset) {
        nameAnalysis(symTab, symTab, offset);
    }
     ...
     * decls in the list.
     */    
    public void nameAnalysis(SymTable symTab, SymTable globalTab, int offset) {
        for (DeclNode node : myDecls) {
            if (node instanceof VarDeclNode) {
                Sym s = ((VarDeclNode)node).nameAnalysis(symTab, globalTab);
		offset -= 4;
		s.setOffset(offset);
		//offset -= 4;
            } else {
...
class FormalsListNode extends ASTnode {
    ...
    public int getSizeParms(){
	return this.sizeParms;
    }
...
class FnBodyNode extends ASTnode {
    ...
    public int getSizeVars(){
	return this.sizeVars;
    }	
    /**
     * nameAnalysis
     * Given a symbol table symTab, do:
     ...
     */
    public void nameAnalysis(SymTable symTab, int offset) {
        myDeclList.nameAnalysis(symTab, offset);
	offset = myDeclList.getOffset();
        myStmtList.nameAnalysis(symTab, offset);
	this.sizeVars = (-1)*myStmtList.getMax();
    }    
...
class StmtListNode extends ASTnode {
	...
	this.offset = offset;
		...
		((IfStmtNode)node).nameAnalysis(symTab, offset);
		int num = ((IfStmtNode)node).getOffset();
		this.offset = num;
		if (num < max){ this.max = num;}
	    }
	    else if (node instanceof IfElseStmtNode){ 
		((IfElseStmtNode)node).nameAnalysis(symTab, offset);
		int num = ((IfElseStmtNode)node).getOffset();
		this.offset = num;
		if (num < max){ this.max = num;}
            }
	    else if (node instanceof WhileStmtNode){ 
		((WhileStmtNode)node).nameAnalysis(symTab, offset);
		int num = ((WhileStmtNode)node).getOffset();
		this.offset = num;
		if (num < max){ this.max = num;}
	    }
	    else {	
    ...
    public void codeGen(){
	   ...
	   if (node instanceof ReturnStmtNode){
		((ReturnStmtNode)node).codeGen(this.returnLabel);
	   }
	   else if (node instanceof IfStmtNode){
		((IfStmtNode)node).setReturnLabel(this.returnLabel);
		((IfStmtNode)node).codeGen();
	   }
	   else if (node instanceof IfElseStmtNode){
		((IfElseStmtNode)node).setReturnLabel(this.returnLabel);
		((IfElseStmtNode)node).codeGen();
	   }
	   else if (node instanceof WhileStmtNode){
		((WhileStmtNode)node).setReturnLabel(this.returnLabel);
		((WhileStmtNode)node).codeGen();
	   }
	   else {
            node.codeGen();
	   }
    ...
    public void setReturnLabel(String s){
	this.returnLabel = s;
    }
...
class VarDeclNode extends DeclNode {
	...
	if (myId.sym().getOffset() == 0){
		...
		Codegen.genLabel(label, ".space 4");
	}
    }
...
class FnDeclNode extends DeclNode {
    ...
    public Sym nameAnalysis(SymTable symTab) {
        ...
        int offset = 0;
        // process the formals
        List<Type> typeList = myFormalsList.nameAnalysis(symTab, offset);
        if (sym != null) {
	...
	int varsSize = myBody.getSizeVars();
	//System.out.println(varsSize);		
	}
	else {
		Codegen.generateWithComment(".text", "");
		String label = "_"+myId.name();
	...
	int parmsSize = myFormalsList.getSizeParms();
	//Codegen.generate("addu", Codegen.FP, Codegen.SP, parmsSize+8);
	Codegen.generate("addu", Codegen.FP, Codegen.SP, 8);
	int varsSize = myBody.getSizeVars()-4;
	//System.out.println(varsSize);
	if (varsSize != 0){
		Codegen.generate("subu", Codegen.SP, Codegen.SP, varsSize);
	}
	
//Codegen.generate("addu", Codegen.FP, Codegen.SP, 8+varsSize);
	...
	/*Codegen.genLabel(exit, "Function exit");
	Codegen.generateIndexed("lw", Codegen.RA, Codegen.FP, (-parmsSize));
	Codegen.generate("move", Codegen.T0, Codegen.FP);
	Codegen.generateIndexed("lw", Codegen.FP, Codegen.FP, (-parmsSize+4));
	...
	System.out.println(-parmsSize+4);*/

	Codegen.genLabel(exit, "Function exit");
	Codegen.generateIndexed("lw", Codegen.RA, Codegen.FP, 0);
	Codegen.generate("add", Codegen.T0, Codegen.FP, parmsSize);
	//Codegen.generateIndexed("lw", Codegen.T0, Codegen.FP, parmsSize);
...
class WriteStmtNode extends StmtNode {
    ...
    public void codeGen(){
	//Int
	if (myType.equals(new IntType()) || myType.equals(new BoolType())){
		myExp.codeGen();
...
class IfStmtNode extends StmtNode {
	...
	this.offset = offset;
        myStmtList.nameAnalysis(symTab, offset);
	this.offset = myStmtList.getOffset();
        try {
            symTab.removeScope();
        } catch (EmptySymTableException ex) {
            System.err.println("Unexpected EmptySymTableException " +
   ...
   public void nameAnalysis(SymTable symTab){
	...
	myStmtList.setReturnLabel(this.returnLabel);
 	myStmtList.codeGen();
...
class IfElseStmtNode extends StmtNode {
    ...
    public void nameAnalysis(SymTable symTab, int offset) {
        myExp.nameAnalysis(symTab);
        symTab.addScope();
        myThenDeclList.nameAnalysis(symTab, offset);
	offset = myThenDeclList.getOffset();
	this.offset = offset;
        myThenStmtList.nameAnalysis(symTab, offset);
	offset = myThenStmtList.getOffset();
	this.offset = offset;
        try {
            symTab.removeScope();
        } catch (EmptySymTableException ex) {
        ...
        symTab.addScope();
        myElseDeclList.nameAnalysis(symTab, offset);
	offset = myElseDeclList.getOffset();
	this.offset = offset;
        myElseStmtList.nameAnalysis(symTab, offset);
	this.offset = myElseStmtList.getOffset();
        try {
            symTab.removeScope();
        } catch (EmptySymTableException ex) {
            System.err.println("Unexpected EmptySymTableException " +
   ...
   public void nameAnalysis(SymTable symTab){
	...
	myThenStmtList.setReturnLabel(this.returnLabel);
 	myThenStmtList.codeGen();
	Codegen.generate("b", doneLabel);
 	Codegen.genLabel(elseLabel);
	myElseStmtList.setReturnLabel(this.returnLabel);
...
class RepeatStmtNode extends StmtNode {
    ...
    public void nameAnalysis(SymTable symTab) {
        ...
        symTab.addScope();
        myDeclList.nameAnalysis(symTab, 0);
        myStmtList.nameAnalysis(symTab, 0);
        try {
            symTab.removeScope();
        } catch (EmptySymTableException ex) {
            System.err.println("Unexpected EmptySymTableException " +
...
class StringLitNode extends ExpNode {
    ...
    public void codeGen(){
	String label = "";
	if (ht.containsKey(myStrVal)){
		label = ht.get(myStrVal);
	}else {
		label = Codegen.nextLabel();
		ht.put(myStrVal, label);
	}
...
class IdNode extends ExpNode {
	...
	if (mySym.getOffset() == 0){
		...
		Codegen.generateIndexed("lw", Codegen.T0, "_"+myStrVal, 0);
	}
	else {	//BEACON seemingly the correct one???
	...
	else {	//BEACON source of the syntax issue
		//Codegen.generate("la", Codegen.T0, name());

		//System.out.println(mySym.getOffset());
		//Codegen.generate("add", Codegen.T1, Codegen.FP, mySym.getOffset());
		//Codegen.generate("lw", Codegen.T1, Codegen.FP, -mySym.getOffset());
		//Codegen.generate("la", Codegen.T0, Codegen.T1);
...
class EqualsNode extends EqualityExpNode {
	...
	String doneLabel = Codegen.nextLabel();
    	// step 3: if T0 == T1
   	Codegen.generate("bne", Codegen.T0, Codegen.T1, falseLabel);	
	Codegen.generate("li", Codegen.T0, Codegen.TRUE);
 	Codegen.genPush(Codegen.T0);
	Codegen.generate("b", doneLabel);
 	Codegen.genLabel(falseLabel);
...
class LessNode extends RelationalExpNode {
	...
	String doneLabel = Codegen.nextLabel();
    	// step 3: if T0 < T1
   	Codegen.generate("bge", Codegen.T0, Codegen.T1, falseLabel);
	Codegen.generate("li", Codegen.T0, Codegen.TRUE);
 	Codegen.genPush(Codegen.T0);
	Codegen.generate("b", doneLabel);
 	Codegen.genLabel(falseLabel);
...
class GreaterNode extends RelationalExpNode {
	...
	String doneLabel = Codegen.nextLabel();
    	// step 3: if T0 > T1
   	Codegen.generate("ble", Codegen.T0, Codegen.T1, falseLabel);	
	Codegen.generate("li", Codegen.T0, Codegen.TRUE);
 	Codegen.genPush(Codegen.T0);
	Codegen.generate("b", doneLabel);
 	Codegen.genLabel(falseLabel);
...
class LessEqNode extends RelationalExpNode {
	...
	String doneLabel = Codegen.nextLabel();
    	// step 3: if T0 <= T1
   	Codegen.generate("bgt", Codegen.T0, Codegen.T1, falseLabel);	
	Codegen.generate("li", Codegen.T0, Codegen.TRUE);
 	Codegen.genPush(Codegen.T0);
	Codegen.generate("b", doneLabel);
 	Codegen.genLabel(falseLabel);
